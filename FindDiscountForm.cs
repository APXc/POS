...
{
    public partial class FindDiscountForm : Form
    {
        public FindDiscountForm()
        {
            InitializeComponent();
        }

        private void FindDiscountForm_Load(object sender, EventArgs e)
        {
            Discount discount = new Discount();
            dataGridView1.DataSource = discount.FindAll();
            dataGridView1.AutoGenerateColumns = true;
            DataRow[] row = discount.FindAll().Select();
            if (row.Length == 0)
            {
                Form form = new DiscountForm();
                form.Show();
                this.Close();
            }
        }
